...
sub uuid_ok {
    my $E = shift;
    our $h4 ||= qr/[0-9A-F]{4}/;
    like($E->uuid, qr/^$h4$h4-$h4-$h4-$h4-$h4$h4$h4$/,
        sprintf "%s: uuid structure", ref($E)
    );
}
...
try {
    my $obj = Abstract->new;
    $line = __LINE__; $obj->abstract_method();
} catch Error with {
    my $E = shift;
    ...
    is("$E", sprintf('Exception: package [%s], filename [%s], line [%s]: called abstract method [%s]',
        __PACKAGE__, $0, $line, 'Abstract::abstract_method'),
        'Error::Hierarchy::Internal::AbstractMethod: stringified exception');
};

try {
    my $foo = Foo->new;
    $line = __LINE__; $foo->test_assert_defined(undef);
} catch Error with {
    my $E = shift;
    ...
    is("$E", sprintf(
        'Exception: package [%s], filename [%s], line [%s]: [%s] called with an undef value',
            __PACKAGE__, $0, $line, 'Foo::test_assert_defined'),
...
try {
    my $foo = Foo->new;
    $line = __LINE__; $foo->test_assert_is_integer(0);
} catch Error with {
    my $E = shift;
    ...
    is("$E", sprintf(
        'Exception: package [%s], filename [%s], line [%s]: [%s] expected an integer value from 1 to 9',
            __PACKAGE__, $0, $line, 'Foo::test_assert_is_integer'),
